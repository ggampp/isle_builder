...
export function createWater(): { mesh: THREE.InstancedMesh; uniforms: { uTime: { value: number } } } {
  ...
  const geo = new THREE.PlaneGeometry(1.06, 1.06);
  ...
  for (let i = 0; i < cells.length; i++) {
    const cell = cells[i];
    if (!cell) continue;
    dummy.position.set(cell.x + 0.5, 0.26, cell.z + 0.5);
    dummy.updateMatrix();
